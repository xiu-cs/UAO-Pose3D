...
def mpjpe_cal(predicted, target): # B, 1,
...
def mpjpe_proj(output_3D, out_target):
    return torch.mean(torch.norm(output_3D - out_target, dim= - 1))
...
def mpjpe_by_action_p1(predicted, target, action, action_error_sum):
    ...
    if len(set(list(action))) == 1: # only one action
        end_index = action[0].find(' ')
        if end_index != -1: # action中不包含空
