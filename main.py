...
def step_Gaussian(split, args, actions, dataLoader, model, optimizer=None, epoch=None):
    ...
    for i, data in enumerate(tqdm(dataLoader, 0)):
        ...
        if split == "train":
            ...
            loss_all["train_loss_gaussian"].update(
                train_loss_gaussian.detach().cpu().numpy() * N, N
            )
            ...
            mu_flip[:, :, :, 0] *= -1
            mu_flip[:, :, args.joints_left + args.joints_right, :] = mu_flip[
            ...
            action_error_sum = test_calculation(
                mu, out_target, action, action_error_sum, args.dataset, subject
            )
...
def test_time_optimization_Gaussian(args, actions, dataLoader, model):
    ...
    for i, data in enumerate(tqdm(dataLoader, 0)):
        ...
        if args.test_time_optimization:
            ...
            mu_flip_pseudo_gt[:, :, args.joints_left + args.joints_right, :] = (
                mu_flip_pseudo_gt[:, :, args.joints_right + args.joints_left, :]
            )
            ...
            for iter_idx in range(args.opt_iter_num):
                ...
                s_flip_gt[:, :, args.joints_left + args.joints_right, :] = s_flip_gt[
                    :, :, args.joints_right + args.joints_left, :
                ]
                output_3D_flip[:, :, :, 0] *= -1
                output_3D_flip[:, :, args.joints_left + args.joints_right, :] = (
                    output_3D_flip[:, :, args.joints_right + args.joints_left, :]
                )
                # gaussian loss for nonflip and flip
                loss_nonflip_gaussian = gaussian_loss(
                    mu_nonflip_pseudo_gt.detach(),
                    s_nonflip_gt.detach(),
                    output_3D_nonflip,
                )  #
                loss_flip_gaussian = gaussian_loss(
                    mu_flip_pseudo_gt.detach(), s_flip_gt.detach(), output_3D_flip
                )  #
                ...
                opt_loss = (
                    loss_nonflip_gaussian + loss_flip_gaussian
                ) * args.weight_gaussian + (
                    loss_nonflip_proj + loss_flip_proj
                ) * args.weight_proj
                opt_z.zero_grad()
                opt_loss.backward()
                opt_z.step()
            ...
            output_3D_flip_z[:, :, args.joints_left + args.joints_right, :] = (
                output_3D_flip_z[:, :, args.joints_right + args.joints_left, :]
            )
...
if __name__ == "__main__":
    ...
    if args.create_file:
        ...
        shutil.copyfile(
            src="model/model_Gaussian.py",
            dst=os.path.join(args.checkpoint, args.create_time + "_model_Gaussian.py"),
        )
        shutil.copyfile(
        ...
        if args.train:
            shutil.copyfile(
                src="train.sh",
                dst=os.path.join(args.checkpoint, args.create_time + "_train.sh"),
            )
        elif args.test:
            shutil.copyfile(
                src="test.sh",
                dst=os.path.join(args.checkpoint, args.create_time + "_test.sh"),
            )

        logging.basicConfig(
            format="%(asctime)s %(message)s",
            datefmt="%Y/%m/%d %H:%M:%S",
            filename=os.path.join(args.checkpoint, "record.log"),
            level=logging.INFO,
        )
    ...
    for epoch in range(1, args.nepoch):
        if args.train:
            e_train_g = train_gaussian(
                args, actions, train_dataloader, model, optimizer, epoch
            )

        if args.test:
            if args.test_time_optimization:
                e_mu_p1, e_mu_p2 = test_time_optimization_Gaussian(
                    args, actions, test_dataloader, model
                )
            else:
            ...
            if args.train and data_threshold < args.previous_best_threshold:
                args.previous_name = save_model(
                    args.previous_name,
                    save_folder,
                    epoch,
                    data_threshold,
                    model["gaussian"],
                    "Model_Gaussian_mu_p1",
                )
            ...
            if args.train == 0:
                ...
                print(
                    "e: %d, lr: %.7f, e_train_g: %.4f, e_mu_p1: %.2f, e_mu_p2: %.2f"
